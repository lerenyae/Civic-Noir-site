...
export default function PostPage({ params }: { params: { slug: string } }) {
  ...
  const renderContent = (content: string) => {
    return content.split("\n\n").map((block, i) => {
      if (block.startsWith("## ")) return <h2 key={i} className="font-serif text-2xl text-bone-200 font-light mt-12 mb-6">{block.replace("## ", "")}</h2>;
      const parts = block.split(/(\*[^*]+\*)/g);
      return (<p key={i} className="text-sm text-bone-300/55 font-light leading-[1.9] mb-6">{parts.map((part, j) => {
        if (part.startsWith("*") && part.endsWith("*")) return <em key={j} className="text-bone-300/70 font-serif text-base">{part.slice(1, -1)}</em>;
        return part;
      })}</p>);
    });
  };
